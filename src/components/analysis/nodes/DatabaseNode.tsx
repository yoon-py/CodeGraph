...
type DatabaseNodeData = GraphNode & { isNew?: boolean; isChanged?: boolean };
...
export function DatabaseNode({ data, selected }: NodeProps) {
  ...
  return (
    <div
      className={`node-card node-database${d.isChanged ? ' is-modified' : ''}${selected ? ' selected' : ''}`}
      onClick={() => setSelectedNodeId(d.id)}
    >
      <Handle type="target" position={Position.Left} />
      {d.isChanged && <span className="node-change-badge modified">Modified</span>}
      <div className="node-header">
        <div className="node-type-icon">D</div>
